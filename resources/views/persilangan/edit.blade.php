...
<div class="container-fluid">
    ...
    <div class="card mb-4">
        <div class="card-body">
            <form action="{{ url('persilangan/update', [$val->kodePersilangan]) }}" class="form-row" method="post">
                @csrf
                <div class="col-md-6 mb-3">
                    <label for="sed">Seed</label>
                    <input type="text" class="form-control" id="sed" list="seed" name="seed" value="{{ $val->seed }}" required>
                    <datalist id="seed">
                        @foreach ($data as $d)
                            @if ($d->jk == "seed")
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control" id="polen" list="pollen" name="pollen" value="{{ $val->pollen }}" required>
                    <datalist id="pollen">
                        @foreach ($data as $d)
                            @if ($d->jk == "pollen")
                                <option value="{{ $d->idTanaman }}">{{ $d->name }}</option>
